<template>
  ...
  <div class="contact">
    ...
    <div class="educations" v-if="educationLists">
      <h5 class="title">your Education</h5>
      <p class="education-text" v-if="educationLists == null">
        add your education here
      </p>

      <ul
        class="educations-lists list-unstyled text-capitalize"
        v-if="educationLists !== null"
      >
        <li class="items" v-for="item in educationLists" :key="item.id">
          ...
          <form class="educate-form">
            ...
            <div class="form-group">
              ...
              <div class="validation-error">
                <span
                  v-if="
                    !$v.educationForm.name.required &&
                    $v.educationForm.name.$dirty
                  "
                  class="text-danger"
...
export default {
  ...
  mounted() {
    ...
    // console.log(this.protfolioId);
  },
  computed: {
    ...mapState({
      educationLists: (state) => state.ProfileModule.userEducationLists,
    }),
  },
  methods: {
    showFormModal() {
    ...
    getEducationItemDetailsForEdit(itemId) {
      ...
      this.$store
        .dispatch("ProfileModule/getSingleEducationToEdit", itemId)
        .then((response) => {
          console.log(response);
          this.educationForm.name = response.name;
          this.educationForm.join_date = response.join_date;
          this.educationForm.leave_date = response.leave_date;
        });
    },
    getUserEducations() {
      this.$store.dispatch("ProfileModule/getUserEducationLists");
    },
    ...
    createEducation(e) {
      ...
      if (this.$v.educationForm.$invalid) {
        ...
        educationData.append("name", this.educationForm.name);
        educationData.append("join_date", this.educationForm.join_date);
        educationData.append("leave_date", this.educationForm.leave_date);
        educationData.append("profile_id", this.protfolioId);
        this.$store
          .dispatch("ProfileModule/setUserEducation", educationData)
          .then(() => {
            this.loading = false;
    ...
    editEducation(e) {
      ...
      if (this.$v.educationForm.$invalid) {
        ...
        educationData.append("join_date", this.educationForm.join_date);
        educationData.append("leave_date", this.educationForm.leave_date);
        educationData.append("_method", this.educationForm._method);
        let payload = { id: this.educationIdWhenEdit, data: educationData };
        this.$store
          .dispatch("ProfileModule/editUserEducation", payload)
          .then(() => {
            this.getUserEducations();
          })
          .finally(() => {
    ...
    deleteEducation(id) {
      this.loading = true;
      this.$store
        .dispatch("ProfileModule/deleteSingleEducation", id)
        .then(() => {
          this.loading = false;
        });
    },
