<template>
  <div class="single-details view-padding" v-if="event">
    ...
    >
      <div class="container">
        <div class="every-everything">
          <h5 class="every-name">{{ event.name }}</h5>
          <p class="every-p">
            <fa class="every-icon" icon="clock" /> {{ event.time }}
          </p>
          <p class="every-p">
      ...
      <div class="row">
        <div class="all-details-things col-md-7 col-lg-8">
          <div class="description">
            <h3 class="single-head section-heading">{{ $t("event.Event") }}</h3>
          ...
          <div class="time-types">
            ...
            >
              ...
              <li class="items">
                ...
                <span class="d-block">{{ event.address }}</span>
              </li>
              <li class="items">
                <h6>{{ $t("event.Category") }} &amp; {{ $t("event.Type") }}</h6>
                <span v-if="event.type" class="d-block">{{
                  event.type.name
                }}</span>
              </li>
            </ul>
          ...
          <div class="organizer-information" v-if="event.organize">
            <div class="row">
              <div class="one col-md-6 col-lg-4">
                <img
                  v-if="event.organize.image"
                  class="img w-100 h-100"
                  :src="event.organize.image"
                  :alt="event.organize.name"
                />
                <img
                  v-else
                  class="img w-100 h-100"
              ...
              <div class="two col-md-6 col-lg-8">
                <h6 class="instruct">
                  <fa class="icons" icon="user" /> {{ event.organize.name }}
                </h6>
                <span class="d-block instruct"
                  ><fa class="icons" icon="envelope" />
                  {{ event.organize.email }}
                </span>
                <span class="d-block instruct"
                  ><fa class="icons" icon="phone-alt" />
                  {{ event.organize.phone }}</span
                >
                <span class="d-block instruct"
                  ><fa class="icons" icon="map-marker-alt" />
                  {{ event.organize.country.name }} /
                  {{ event.organize.city.name }}</span
                >
                <span class="d-block instruct"
                  ><fa class="icons star" icon="star" />
                  {{ event.organize.rate }}</span
                >
              </div>
            </div>
            <div class="following">
              <button
                @click="testing(event.organize.id)"
                class="follow-company details-button"
              >
                {{ $t("global.Follow Company") }}
              </button>
              <!-- <button
                @click="followCompany(event.organize.id)"
                class="follow-company details-button"
              >
                {{ $t("global.Follow Company") }}
              </button> -->
          ...
          <div class="event-ratings-and-reviews">
            <!-- Start All Comments  -->
            <Reviews :reviews="event.comment"></Reviews>
            <div class="pharma-add-rate-form-container">
              ...
              <form
                ...
                action=""
                @submit.prevent="addReview(event.id)"
              >
            ...
            <!-- End All Comments  -->
            <!-- Start All Rating -->
            <Ratings :ratings="event.rate"></Ratings>
            <div class="pharma-add-rate-form-container text-capitalize">
              ...
              <form
                ...
                @submit.prevent="addRate(event.id)"
              >
                <div class="form-group">
                  <textarea
                    class="textarea input-focus d-block"
        ...
        <div class="top-rated-companies col-md-5 col-lg-4">
          <h5 class="section-heading text-capitalize">
            {{ $t("event.related") }} <span>{{ $t("event.Events") }}</span>
          </h5>
          <div
            class="top-company"
            v-for="item in topRelatedEvents"
            :key="item.id"
          >
...
export default {
  ...
  computed: {
    ...mapState({
      event: (state) => state.EventsAndNewsModule.EventsModle.singleEvent,
      topRelatedEvents: (state) =>
        state.EventsAndNewsModule.EventsModle.realetdEvents,
    }),
    showErrors() {
      return this.error;
    },
  ...
  methods: {
    singleEvent() {
      this.$store.dispatch(
        "EventsAndNewsModule/EventsModle/getSingleEvent",
        this.$route.params.id
      );
    },
    relatedEvents() {
      this.$store.dispatch(
        "EventsAndNewsModule/EventsModle/getRelatedvents",
        this.$route.params.id
      );
    },
    followCompany(id) {
      ...
      this.$store
        ...
        .catch((error) => {
          // this.error = error.response.data.errors.company_id;
          console.log(error);
    ...
    testing(id) {
      ...
      //   });
      testingMethods("followCompany", { company_id: id })
        .then((response) => {
          console.log(response);
        })
        .catch((error) => {
